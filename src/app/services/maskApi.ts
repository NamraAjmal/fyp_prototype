...
// Stream frame detection — sends base64 frame, gets result back
// FIXED: Using the correct endpoint that matches your backend (/mask-detect)
export async function detectMaskStream(
  frameBase64: string,
  cameraId = "cam_01",
  location = "Unknown Site"
  ...
  console.log(
    `[API] Sending frame for detection, length: ${frameBase64.length} chars`
  );
  ...
  if (!res.ok) {
    console.error(`[API] Stream detection failed: ${res.statusText}`);
    throw new Error(`Stream detection failed: ${res.statusText}`);
  }

  const result = await res.json();
  console.log(`[API] Detection response:`, {
