  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const mockResults = filesArray.map((file, index) => ({
        id: Date.now() + index,
        fileName: file.name,
        personsDetected: Math.floor(Math.random() * 6) + 1,
        masksDetected: Math.floor(Math.random() * 5),
        compliance: Math.random() > 0.15,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-600">
            Upload images or use live camera for mask detection
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-md border border-slate-200/50">
        ...
        {captureMode === "upload" && (
          <div>
            <div className="border-2 border-dashed border-slate-300 rounded-lg p-12 hover:border-teal-500 transition-colors cursor-pointer">
              <input
                type="file"
                multiple
                accept="image/*"
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium shadow-lg transition-all ${
                  isRecording
                    ...
                    : "bg-gradient-to-r from-teal-500 to-green-500 hover:shadow-xl text-white"
                }`}
              >
                {isRecording ? (
                  <>
              ...
              >
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-600">
                    {result.personsDetected} person(s) • {result.masksDetected}{" "}
                    mask(s) • Confidence: {result.confidence}%
                  </p>
                </div>
